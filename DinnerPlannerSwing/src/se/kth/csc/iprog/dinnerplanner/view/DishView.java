...
public class DishView extends JPanel {
	...
	public DishView(DinnerModel model, Dish dish) {
		...
		JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, true, new PreparationView(dish), new IngredientView(dish.getIngredients()));
		...
		JLabel lblPriceFor = new JLabel("$ "+model.getDishPrice(dish)+" for "+model.getNumberOfGuests()+" persons");
    ...
    public void paintComponent(Graphics g) {
        ...
        g.drawImage(bi, 15, 15, 100, 100, null); // see javadoc for more info on the parameters            
    }
}
